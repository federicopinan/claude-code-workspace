...
Read relevant reference files in `referencia/seguridad/` for comprehensive patterns. Apply security principles to whatever code you're currently writing or reviewing.

When reviewing existing code, scan for vulnerability patterns and flag each finding with severity (Critical/High/Medium/Low) and a concrete fix.